...
async function sendOwnerInquiryEmail({ ownerEmail, ownerName, senderName, senderPhone, senderEmail, listingTitle, message, subject }) {
  ...
  await transporter.sendMail({
    ...
    to: ownerEmail,
    subject: `New enquiry on your listing: ${escHtml(listingTitle)}`,
    html: `
      <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:24px;border:1px solid #e8e3da;border-radius:4px">
        ...
        <p style="font-size:15px"><strong>Listing:</strong> ${escHtml(listingTitle)}</p>
        <p style="font-size:15px"><strong>Subject:</strong> ${escHtml(subject) || 'Property Enquiry'}</p>
        <p style="font-size:15px"><strong>From:</strong> ${escHtml(senderName)}</p>
        ${senderPhone ? `<p style="font-size:15px"><strong>Phone:</strong> ${escHtml(senderPhone)}</p>` : ''}
        ${senderEmail ? `<p style="font-size:15px"><strong>Email:</strong> ${escHtml(senderEmail)}</p>` : ''}
        <div style="background:#f7f4ef;border-left:3px solid #b89b6a;padding:16px;margin:20px 0;border-radius:2px">
          <p style="margin:0;font-size:15px;color:#333">${escHtml(message)}</p>
        </div>
...
async function sendEnquiryConfirmationEmail({ toEmail, toName, listingTitle }) {
  ...
  await transporter.sendMail({
    from: `"NestIQ" <${process.env.EMAIL_USER}>`,
    to: toEmail,
    subject: `Your enquiry has been received — ${escHtml(listingTitle)}`,
    html: `
      <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:24px;border:1px solid #e8e3da;border-radius:4px">
        <h2 style="color:#b89b6a;font-family:Georgia,serif">Message Received</h2>
        <p style="font-size:15px">Hi ${escHtml(toName)},</p>
        <p style="font-size:15px">We've forwarded your enquiry about <strong>${escHtml(listingTitle)}</strong> to the property owner. They'll be in touch with you shortly.</p>
...
async function sendContactFormEmail({ senderName, senderEmail, senderPhone, subject, message }) {
  ...
  await transporter.sendMail({
    ...
    subject: `[NestIQ Contact] ${escHtml(subject) || 'New Message'}`,
    html: `
      <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:24px;border:1px solid #e8e3da;border-radius:4px">
        ...
        <hr style="border:none;border-top:1px solid #e8e3da;margin:20px 0"/>
        <p style="font-size:15px"><strong>Name:</strong> ${escHtml(senderName)}</p>
        ${senderEmail ? `<p style="font-size:15px"><strong>Email:</strong> ${escHtml(senderEmail)}</p>` : ''}
        ${senderPhone ? `<p style="font-size:15px"><strong>Phone:</strong> ${escHtml(senderPhone)}</p>` : ''}
        <p style="font-size:15px"><strong>Subject:</strong> ${escHtml(subject) || '—'}</p>
        <div style="background:#f7f4ef;border-left:3px solid #b89b6a;padding:16px;margin:20px 0;border-radius:2px">
          <p style="margin:0;font-size:15px;color:#333">${escHtml(message)}</p>
        </div>
